export function renderUI(profiles, running) {
...
export function connectWebSocket() {
    ...
    document.getElementById("apply-tune")?.addEventListener("click", () => {
        const preset = document.getElementById("tune-preset").value;
    ...
    document.getElementById("set-waypoint")?.addEventListener("click", () => {
        const x = parseInt(document.getElementById("waypoint-x").value);
        const y = parseInt(document.getElementById("waypoint-y").value);
        sendCommand(ws, { command: "execute", params: { task: "navigate_to_waypoint", x, y } });
    });
    document.querySelectorAll(".toggle-switch input").forEach(input => {
        input.addEventListener("change", (e) => {
            const profile = e.target.parentElement.previousElementSibling.textContent;
            sendCommand(ws, {
                command: "execute",
                params: { task: "select_profile", profile, state: e.target.checked }
            });
        });
    });
}
